...
- The authoritative key set is **derived mechanically from the `config` decode-shape structs' `toml` tags** (`fileShape`, `releaseShape`, `commitShape`, `hooksShape`) — **not** a hand-maintained list — so the test catches a genuine schema↔SoT divergence rather than comparing two copies of the same hand-list. The bijection is total over **leaf keys**: every `toml`-tagged **leaf** field (scalar or collection) has exactly one matching SoT row at its level, and every SoT row maps back to a leaf field. (The exact reflection helper / package layout is a planning detail; the *contract* — derive from the schema, match per (level, key), bijective over leaf keys — is decided.)
- **Sub-table container fields are recursed, not counted.** Three `toml`-tagged fields are table containers, not keys: `fileShape.Release` (`toml:"release"`), `fileShape.Commit` (`toml:"commit"`), and `releaseShape.Hooks` (`toml:"hooks"`). The traversal **recurses into** these — the nested struct's tag supplies the `level` (`[release]`, `[commit]`, `[release.hooks]`) for the leaf rows inside — and emits **no** SoT row for `release`/`commit`/`hooks` themselves. This recurse-don't-count rule for sub-shape fields is part of the decided contract; it is the inverse of the dual-level case above (a container field maps to **zero** rows plus recursion, whereas a dual-level key like `ai_command` maps to one row per level).
