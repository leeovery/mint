...
Mint is a Go CLI that cuts releases (`mint release`), regenerates release notes (`mint release regenerate`), mints AI-generated commits (`mint commit`), and emits an AI-agent setup guide (`mint setup`). See README.md for the user-facing surface. This file is the working contract for changing the code.
...
- `internal/config` — the canonical `.mint.toml` schema, strict decoding, and the config-metadata SoT (`MetadataRows()`: one row per `(level, key)` carrying key·level·default·description, drift-tested against the decode-shape structs). `internal/initgen` — the pure `mint init` generator (now a minimal empty-body + dual-pointer header template; `ReleaseShim()` unchanged).
- `internal/setupguide` — the embedded `mint setup` guide emitter: renders the SoT config reference plus the marker-anchored guide sections (pipeline/hook model, etiquette, minimalism, existing-config/upgrade). `internal/configtest` — test-only seam indexing the SoT by `(level, key)` (never linked into the binary).
...
6. **Config is strict.** All keys live in `internal/config`'s canonical schema; `DisallowUnknownFields` rejects unknowns; new keys need a `typeErrorMessages` entry for a friendly type error. `ai_command` and `timeout` live at BOTH levels — a top-level shared baseline plus an optional per-verb override — resolved per-key `[verb] → shared → shipped default` by the typed `AICommandFor`/`TimeoutFor` accessors (keyed on the closed `Verb` enum, which has no `regenerate` value — regenerate rides on `[release]`); `max_diff_lines`/`diff_exclude` stay shared-only at the top level. Other verb keys are namespaced (`[release]`, `[commit]`). A new config key must be added to the config-metadata SoT (`config.MetadataRows()`) — the drift test in `internal/config` fails the build until the SoT and the decode-shape schema biject over `(level, key)` leaf pairs — and to the README's per-key tables (a tripwire test asserts every schema key name appears in the README). The `internal/initgen` template stays minimal (empty body + header) and carries no per-key entries, so a new key is NOT added there.
